...
const NewsCard: FC<INewsCard> = ({uri, title, paragraph, featuredImage}) => {
	return (
		<div
			...
			>
				<Link href={uri ? `news-insights${uri}` : `/`}>
					<motion.h2
						...
						viewport={{once: true}}
						className="mb-2 text-lg font-semibold text-pureBlack transition-all ease-in-out duration-200 hover:text-blue-default"
					>
						{title}
					</motion.h2>
				</Link>
