...
const EnquiryRequestForm: FC<IEnquiryRequestForm> = ({
	title,
	contentOneOpen,
	contentTwoOpen,
	contentThreeOpen,
	...
	const validate: any = (values: any) => {
		...
		if (!values?.phoneNumber) {
			errors.phoneNumber = "Requiblue*";
		} else if (values?.phoneNumber.length <= 0) {
			errors.phoneNumber = "Please provide us your phone number.";
		}
		if (!values?.subject) {
			errors.subject = "Requiblue*";
		} else if (values?.subject.length <= 0) {
			errors.subject = "Please inform us about the topic.";
		}

		if (!values?.message) {
			errors.message = "Requiblue*";
		...
		<>
			<Formik onSubmit={formik?.onSubmit} initialValues={formik?.initialValues}>
				<Form className={styles.enquiryRequestForm + " w-full shadow-12xl"}>
					...
					>
						<div className="grid grid-cols-1 md:grid-cols-3 gap-2 items-center justify-between">
							<motion.div
								initial={initial}
...
export default EnquiryRequestForm;
